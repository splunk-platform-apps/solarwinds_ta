...
def stream_events(inputs: smi.InputDefinition, event_writer: smi.EventWriter):
    for input_name, input_item in inputs.inputs.items():
        ...
        try:
            ...
            server = get_account_property(
                session_key, input_item.get("account"), "server"
            )
            username = get_account_property(
                session_key, input_item.get("account"), "username"
            )
            password = get_account_property(
                session_key, input_item.get("account"), "password"
            )
            ...
            if response.status_code == 200:
                ...
                for event in data:
                    if checkpoint_field not in event:
                        raise Exception(f"{checkpoint_field} not found in the event: {event}")
                    ...
                    if kvstore_checkpointer.get(checkpoint) is None:
                        ...
                        logger.debug(
                            f"Checkpoint already exists for {checkpoint}, skipping event"
                        )
